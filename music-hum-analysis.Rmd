...
#full_join(track_features, track_features2)
...
df <- spotify_data %>%
  select(popularity, valence, speechiness, tempo, artists, Genre) %>%
  group_by(Popularity)%>%
  filter(!is.na(Popularity)) %>%
  filter(!is.na(Valence.))%>%
  filter(!is.na(Speechiness.))%>%
  filter(!is.na(Beats.Per.Minute))%>%
  filter(!is.na(Artist.Name))%>%
  ggplot(mapping = aes(x = Valence., y = Popularity, size = Beats.Per.Minute, color = Genre))+
